        ...
        <div class="card-body">
            <div class="dt-responsive table-responsive">
                <table class="table table-sm  nowrap mb-2 dataTable">
                    ...
                    <tbody>
                        @if ($commentList->count() > 0 && $productList->count() > 0)
                            @foreach ($commentList as $data)
                                <tr>
                                    ...
                                    <td>{{ isset($data->commentable->name) ? $data->commentable->name : 'N/A' }}</td>
                                    <td>
                                        <div class="d-inline-block align-middle">
                                            <div class="d-inline-block">
                                                <span class="m-b-0">
                                                    <span><i
                                                            class="feather {{ $data->rating >= 1 ? 'icon-star-on' : 'icon-star' }} text-warning"></i></span>
                                                    <span><i
                                                            class="feather {{ $data->rating >= 2 ? 'icon-star-on' : 'icon-star' }} text-warning"></i></span>
                                                    <span><i
                                                            class="feather {{ $data->rating >= 3 ? 'icon-star-on' : 'icon-star' }} text-warning"></i></span>
                                                    <span><i
                                                            class="feather {{ $data->rating >= 4 ? 'icon-star-on' : 'icon-star' }} text-warning"></i></span>
                                                    <span><i
                                                            class="feather {{ $data->rating >= 5 ? 'icon-star-on' : 'icon-star' }} text-warning"></i></span>
                                                </span>
                                                <p class="m-b-0">
                                                    {{ Illuminate\Support\Str::limit($data->message, 30) }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        ...
                                        <div class="overlay-edit">
                                            ...
                                            <span data-toggle="modal" data-target="#commentModal">
                                                <button class="btn btn-icon btn-info"
                                                    wire:click="show({{ $data->id }})" type="button"
                                                    data-toggle="tooltip" title="Detalle">
                                                    <i class="feather icon-list"></i></button>
                                            </span>

                                            @if ($data->status == 1)
                                                <button wire:click="deactivate({{ $data->id }})"
                                                    data-toggle="tooltip" title="Ocultar" type="button"
                                                    class="btn btn-icon btn-danger">
                                                    <i class="feather icon-eye-off"></i>
                                                </button>
                                            @else
                                                <button wire:click="activate({{ $data->id }})"
                                                    data-toggle="tooltip" title="Mostrar" type="button"
